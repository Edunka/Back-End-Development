...
if (isset($_POST['submit'])){
  ...
  $sql = "UPDATE TeacherAssistant SET  AssistantAddress = '$address', Gender = '$gender', Telephone = '$number',
  Email = '$email', Salary = '$salary', BackgroundCheck = '$background', DinnerMoney = '$dinner', LibraryBooks = '$books' WHERE AssistantID = '$id' ";
...
<?php
  if (isset($_POST['delete'])){
    ...
    $aid = $_POST['aid'];
    $sql = "DELETE FROM TeacherAssistant WHERE AssistantID = '$aid' ";
...
<head>
    ...
    <title>View & Edit Teacher Assistant</title>
</head>
<body>
 <!-- navbar -->
      ...
      <div class="table-responsive">
        <table class="table table-striped">
            ...
            <tbody>
                ...
                $sql = mysqli_query($link, "SELECT  AssistantID, AssistantName, Birthday, Gender, AssistantAddress,
                Email, Telephone, DinnerMoney, LibraryBooks, Salary, BackgroundCheck, TeacherID FROM TeacherAssistant");
                while ($row = $sql->fetch_assoc()){
                echo "
                <tr>
                    <th>{$row['AssistantID']}</th>
                    <th>{$row['AssistantName']}</th>
                    <th>{$row['Birthday']}</th>
                    <th>{$row['Gender']}</th>
                    <th>{$row['AssistantAddress']}</th>
                    <th>{$row['Email']}</th>
                    <th>{$row['Telephone']}</th>
                    <th>{$row['DinnerMoney']}</th>
                    <th>{$row['LibraryBooks']}</th>
                    <th>{$row['Salary']}</th>
...
<!-- Update -->
      <form action="view-edit-assistant.php" method="post" class="container-fluid text-center w-50 mb-2 border border-dark border-5 p-2 rounded">
            <h2>UPDATE A TEACHER ASSISTANT</h2>
            <div class="">
              <label for="id">ID</label>
              <input type="text" class="form-control" id="id" placeholder="Assistant ID"
                  alt="id" name="id">
          </div>
              ...
              <div class="">
                  ...
                  <input type="number" class="form-control" id="number" name="number"
                      placeholder="TelePhone Number" alt="telephone number" required>
              </div>

              <div class="">
                  <label for="email">Email</label>
                  <input type="text" class="form-control" id="email" name="email"
                      placeholder="Someone@email.co.uk" alt="email" required>
              </div>

              <div class="">
                  <label for="salary">salary</label>
                  <input type="text" class="form-control" id="salary" name="salary"
                      placeholder="Salary" alt="salary" required>
              </div>

              <div class="">
                  <label for="background">Background Check</label>
                  <input type="text" class="form-control" id="background" name="background"
                      placeholder="Background Check" alt="background check" required>
              </div>

              <div class="">
                  <label for="dinner">Dinner Money</label>
                  <input type="text" class="form-control" id="dinnermoney" name="dinnermoney"
                      placeholder="Dinner Money" alt="Dinner Money" required>
              </div>

              <div class="">
                  <label for="books">Library Books</label>
                  <input type="text" class="form-control" id="librarybooks" name="librarybooks"
                      placeholder="Library Books" alt="Library Books" required>
              </div>
              <br>
          ...
          action="view-edit-assistant.php" method="post">
            <h2>Delete A Assistant</h2>
            <label for="aid">AssistantID</label>
              <select name="aid">
                
    <?php
    // shows the available id's within that data table
    $sql = mysqli_query($link, "SELECT  AssistantID FROM TeacherAssistant");
    while ($row = $sql->fetch_assoc()){
    echo "
    
        <option>{$row['AssistantID']}</option>
