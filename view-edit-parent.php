...
if (isset($_POST['submit'])){
  ...
  $sql = "UPDATE Parents SET  ParentAddress = '$address', Gender = '$gender', ParentEmail = '$email', ParentPhone = '$number'  WHERE ParentID = '$id' ";
      ...
      <div class="table-responsive">
        <table class="table table-striped">
            ...
            <tbody>
                ...
                $sql = mysqli_query($link, "SELECT ParentID, PupilID, ParentName, Gender, Birthday, 
                ParentAddress, ParentEmail, ParentPhone FROM Parents");
...
<!-- update form -->
      <form action="view-edit-parent.php" method="post" class="container-fluid text-center w-50 mb-2 border border-dark border-5 p-2 rounded">
          ...
          action="view-edit-parent.php" method="post">
          <h2>Delete A Parent</h2>
          <label for="pid"></label>
          <select name="pid">
                
    <?php
      // shows the available id's within that data table
    $sql = mysqli_query($link, "SELECT ParentID  FROM Parents");
    while ($row = $sql->fetch_assoc()){
    echo "
    
        <option>{$row['ParentID']}</option>
